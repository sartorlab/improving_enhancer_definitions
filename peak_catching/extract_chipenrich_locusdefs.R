...
write.table(fivekb_df, file = gzfile('../locusdefs/5kb.ldef'), sep='\t', col.names = T, row.names = F, quote = F
write.table(fivekb_outside_df, file = gzfile('../locusdefs/5kb_outside.ldef'), sep='\t', col.names = T, row.names = F, quote = F
write.table(ntss_df, file = gzfile('../locusdefs/nearest_tss.ldef'), sep='\t', col.names = T, row.names = F, quote = F)
