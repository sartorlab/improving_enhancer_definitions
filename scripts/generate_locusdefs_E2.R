...
combinations_E2 = expand.grid(chromhmm_pool, dnase_pool, thurman_pool, fantom_pool, expand_pool, P2P_pool, E2_pool, thurman_pool, fantom_pool, stringsAsFactors=F)

# Get rid of 4 NAs in 1-4 and 6-9
combinations_E2 = combinations_E2[!apply(combinations_E2, 1, function(row){all(is.na(row[1:4]))}),]
combinations_E2 = combinations_E2[!apply(combinations_E2, 1, function(row){all(is.na(row[6:9]))}),]

# Get rid of link methods without E2

combinations_E2 <- combinations_E2[apply(combinations_E2[,6:9],1,function(x){any(x %in% "E2")}),]

# get rid of link methods containing "thurman" or "fantom"
# combinations_E2 = combinations_E2[apply(combinations_E2[,6:9],1,function(x){!(any(x %in% c("thurman","fantom"))) & any(x %in% "E2")}),]

# Make codes
enhancer_codes = apply(combinations_E2[,1:4], 1, function(row){paste(row[!is.na(row)], collapse='_')})
extension_codes = combinations_E2[,5]
...
for(base in enhancer_base) {
	...
	message('Reading E...')
	base_e = read.table(file = sprintf('%s/%s.E2.pairs', '/home/qinting/latte/share/improving_enhancer_definitions/pair_lists', base), sep='\t', header = T, as.is = T)
  ...
  nearest_All_pool = c(NA, 'nearest_All')
	interaction_combinations = expand.grid(P2P_pool, E2_pool, thurman_pool, fantom_pool, nearest_pool, nearest_All_pool, stringsAsFactors=F)
  # get rid of interactions without E2
  interaction_combinations = interaction_combinations[apply(interaction_combinations, 1, function(row){!(all(is.na(row))) & any(row %in% "E2")}),]
